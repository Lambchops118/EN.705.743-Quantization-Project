...
- Python 3.10+
- PyTorch 2.7+ for NVIDIA Blackwell GPUs such as `sm_120`
...
If S3 output is enabled, the job still writes local files first for in-container reads and cleanup logic, then uploads the finished artifacts to your bucket immediately after each write.
...
The Docker image now uses `pytorch/pytorch:2.7.1-cuda12.8-cudnn9-runtime`, which is the minimum stack in this repo intended to work on Blackwell-class GPUs.
...
docker run --gpus all --rm \
  ...
  -e MQ_DEVICE="cuda:0" \
  -e MQ_OUTPUT_S3_BUCKET=your-bucket \
  -e MQ_OUTPUT_S3_PREFIX=runai/${HOSTNAME} \
...
S3 output can be configured either in [config/default.yaml](/mnt/c/Users/aljac/Desktop/Model-Quantizer/config/default.yaml:1) under `outputs.s3` or through environment variables, with env vars taking precedence:

- `MQ_OUTPUT_S3_ENABLED=1`
- `MQ_OUTPUT_S3_BUCKET`
- `MQ_OUTPUT_S3_PREFIX`
- `MQ_OUTPUT_S3_REGION`
- `MQ_OUTPUT_S3_ENDPOINT_URL`

When S3 output is enabled, the following files are uploaded:

- per-run logs under `logs/`
- quantization metadata under `artifacts/metadata/`
- benchmark summaries and predictions under `artifacts/benchmarks/`
- per-artifact manifests under `models/quantized/.../quantization_manifest.json`
...
For Vast.ai or Run:ai, use the same image and pass the environment variables in the instance template or startup command. Mount persistent storage for `models/`, `artifacts/`, and `logs/` so downloads and benchmark results survive instance restarts.
