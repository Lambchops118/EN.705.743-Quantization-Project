...
# Add command line arguments. Making this as general purpose as possible so I can use it to quantize models in the future for my own projects.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.run_benchmarks:
        ...
        for item in results:
            print(
                ...
                f"{item['status']} | summary={item.get('summary_uri') or item['summary_path']}"
            )
    ...
    for item in results:
        print(
            ...
            f"log={item.get('log_uri') or item['log_file']}"
        )
    return 0 if error_count == 0 else 1
