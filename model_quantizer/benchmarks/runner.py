...
from model_quantizer.outputs import build_output_publisher
...
class BenchmarkRunner:
    ...
    def __init__(self, config: ProjectConfig) -> None:
        ...
        self.output_publisher = build_output_publisher(config)
            ...
            for row_index, row in enumerate(rows):
                ...
                examples.append(
                    BenchmarkExample(
                        ...
                        choices=self._format_mmlu_answer_choices(choices),
    ...
    @staticmethod
    def _format_mmlu_answer_choices(choices: List[str]) -> List[str]:
        labels = ["A", "B", "C", "D"]
        return [
    ...
    def _evaluate_benchmark(
        ...
        *,
        ...
        summary = {
            ...
            "normalized_load_seconds": round(bundle.normalized_load_seconds, 3),
            "load_breakdown": bundle.load_breakdown,
            ...
            "use_chat_template": self.config.benchmarks.use_chat_template,
            "source_metrics": source_metrics,
        ...
        if self.output_publisher.enabled:
            summary["summary_uri"] = self.output_publisher.reference_for(output_paths["summary"])
            summary["predictions_uri"] = self.output_publisher.reference_for(
    ...
    def _write_benchmark_outputs(self, summary: Dict[str, Any]) -> None:
        ...
        if self.output_publisher.enabled:
            try:
                self.output_publisher.upload_files((summary_path, predictions_path))
            except Exception as exc:
                print(
                    f"[output] upload skipped for {summary_path.parent}: {exc}",
                    flush=True,
                )
