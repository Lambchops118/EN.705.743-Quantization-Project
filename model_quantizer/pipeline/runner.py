...
from model_quantizer.utils.logging_utils import build_pair_logger, close_logger
    ...
    def run(self, selection: PipelineSelection) -> List[Dict[str, object]]:
        ...
        for model_name in selection.model_names:
            ...
            for quantizer_name in selection.quantizer_names:
                ...
                log_reference = (
                    self.output_publisher.reference_for(log_path)
                ...
                try:
                    ...
                    if log_reference:
                        manifest["log_uri"] = log_reference
                    ...
                    write_artifact_metadata(
                        ...
                        manifest,
                        publisher=self.output_publisher,
                    )
                    logger.info(
                    ...
                    if log_reference:
                        failure_manifest["log_uri"] = log_reference
                    write_artifact_metadata(
                        layout.manifest_path,
                        layout.summary_metadata_path,
                        failure_manifest,
                    ...
                    close_logger(logger)
                    if self.output_publisher.enabled and log_path.exists():
                        self.output_publisher.upload_file(log_path)
